...
def split_merged_green_regions(frame, green_mask):
    ...
    for i, cnt in enumerate(contours):
        ...
        # ✅ Fix: if no inner contours or blurred region, use center of mass
        if not inner_cnts:
        ...
        for j, ic in enumerate(inner_cnts):
            ...
            detections.append({'box': [x+ix, y+iy, x+ix+iw, y+iy+ih], 'id': f'p{i+1}-{j+1}', 'parent': i})

        # if none of the inner contours qualify, still add a center
        if not valid_inner:
            M = cv2.moments(roi_mask)
            if M["m00"] > 0:
                cx = x + int(M["m10"] / M["m00"])
...
def get_final_coordinates(detections):
    ...
    for i in range(len(detections)):
        for j in range(i + 1, len(detections)):
            same_parent = (
                'parent' in detections[i]
                and 'parent' in detections[j]
                and detections[i]['parent'] == detections[j]['parent']
            )
            if same_parent or iou(detections[i]['box'], detections[j]['box']) > 0:
                adj[i].append(j)
                adj[j].append(i)
    ...
    for group in groups:
        ...
        if len(group) == 1:
            box = boxes[0]
            if 'center' in detections[group[0]]:
                cx, cy = detections[group[0]]['center']
            else:
                cx = (box[0] + box[2]) / 2.0
                cy = (box[1] + box[3]) / 2.0
