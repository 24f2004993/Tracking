...
# ---------------- CONFIG ----------------
INPUT_DIR = 'path/to/project/1173_frames'
OUTPUT_DIR = "/Users/tanishq/Desktop/particles"
MIN_GREEN_AREA = 20
SPLIT_MIN_AREA = 8
MAX_DISTANCE = 30.0
IOU_MIN = 0.1
...
class DetectionKalmanTracker:
    def __init__(self, max_distance=MAX_DISTANCE, iou_min=IOU_MIN):
        ...
        self.next_id = 21  # <-- MODIFIED: IDs will now start at 21
        self.active_tracks = {}  # id -> (bbox, centroid, KalmanTrack)
        self.trajectories = {}   # id -> list of centroids
    ...
    def update(self, prev_tracks, detections):
        ...
        for r, c in matches:
            ...
            # Check if trajectory list exists before appending
            if tid not in self.trajectories:
                self.trajectories[tid] = []
            self.trajectories[tid].append(corrected_centroid)
        ...
        for c in unmatched_det:
            ...
            self.trajectories[tid] = [centroid]
...
def run_tracker():
    ...
    for i, fp in enumerate(frames):
        ...
        # Draw trajectories
        for tid, path in tracker.trajectories.items():
            if tid in tracks: # Only draw for currently active tracks
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for tid, path in tracker.trajectories.items():
            for frame_idx, pos in enumerate(path):
                writer.writerow([tid, frame_idx, pos[0], pos[1]])
    print("Trajectories saved to CSV:", csv_path)
